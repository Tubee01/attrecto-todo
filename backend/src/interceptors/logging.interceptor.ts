...
export class LoggingInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler) {
    ...
    const requestLog = await this.insertLog(
      method,
      `${context.getClass().name}_${context.getHandler().name}_REQUEST`,
      user.id,
      requestData,
    );
    return next.handle().pipe(
      tap((data) => {
        this.insertLog(
          method,
          `${context.getClass().name}_${context.getHandler().name}_RESPONSE`,
          user.id,
          data,
          requestLog,
        );
      ...
      catchError((error) => {
        ...
        this.insertLog(
          method,
          `${context.getClass().name}_${context.getHandler().name}_ERROR`,
          user.id,
          { status, message },
          requestLog,
        );
  ...
  async insertLog(
    ...
    const type = logType[method.toUpperCase()];
    const query =
      'INSERT INTO "Log" (type, handler, data, user_id, parent_id) VALUES ($1, $2, $3, $4, $5) RETURNING *';
