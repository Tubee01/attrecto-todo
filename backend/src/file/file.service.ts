...
export class FileService {
    ...
    processImage(file: Express.Multer.File, userId: string = ''): void {
        const { originalname, mimetype, buffer } = file;
        const childFork = fork(path.resolve(__dirname, './image.process'));
        childFork.on('message', async msg => {
            childFork.kill();
            const fileId = await this.saveFile(originalname.split('.')[0], mimetype, msg.data);
            if (isUUID(userId)) {
                const query = 'UPDATE "User" SET profile_image_file_id = $1 WHERE id = $2';
    ...
    async saveFile(filename: string, mimeType: string, buffer: Buffer): Promise<string> {
        const query = 'INSERT INTO "File" (name, file , mime_type) VALUES ($1, $2 , $3) RETURNING *';
