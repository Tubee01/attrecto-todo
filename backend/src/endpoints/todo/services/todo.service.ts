...
import { CONNECTION, USER } from 'src/constants';
...
export class TodoService {
    ...
    async create(body: CreateTodoDTO) {
        const { title, description = null, userId = this.sessionUser.id } = body;
        const user = await this.userService.findByUnique(userId);
        if (!user) {
            throw new HttpException('User not found', HttpStatus.BAD_REQUEST);
        }
        const query = 'INSERT INTO "Todo" (title, description, user_id) VALUES ($1, $2 , $3) RETURNING *';
    ...
    async update(id: string, body: CreateTodoDTO) {
        ...
        const user = await this.userService.findByUnique(userId || this.sessionUser.id);
        if (!user) {
            throw new HttpException('User not found', HttpStatus.BAD_REQUEST);
        }
        ...
        const { query, values } = buildUpdateQuery('Todo', { title, description, ...rest }, id);
        const { rows } = await this.pgClient.query(query, values);
